...
void ChangeSize(int nWidth, int nHeight)
{
    ...
    // 并将其加载到投影矩阵堆栈上
    projectionMarix.LoadMatrix(viewFrustum.GetProjectionMatrix());
    // 设置变换管道以使用两个矩阵堆栈（变换矩阵modelViewMatrix,投影矩阵projectionMatrix）
    // 初始化 GLGeometryTransform的实例transformPipeline，通过将它的内部指针设置为模型视图矩阵堆栈和投影矩阵堆栈实例，来完成初始化
    // 这个操作也可以在SetupRC中完成，但在窗口大小改变时或者窗口创建时设置它们并没有坏处，而且这样可以一次性完成矩阵和管线的设置
    transformPipeline.SetMatrixStacks(modelViewMatrix, projectionMarix);
